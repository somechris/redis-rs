...
pub struct RedisServer {
    ...
    /// liveness, and [`stop_with_infos`](Self::stop_with_infos) yields relevant process infos for
    /// error messages.
...
impl RedisServer {
    ...
    ///
    /// If `format_process_infos` is `true` for an instance's first call to this method, the
    /// returned string holds the formatted process information (exit code, ...), which is useful
    /// to build error messages.
    fn stop_internal(&mut self, format_process_infos: bool) -> String {
        if let Some(mut process) = self.process.take() {
            ...
            // Wait for the process to stop and collect its infos (if needed)
            let infos = if format_process_infos {
                let exit_output_infos = match process.wait_with_output() {
                    Ok(output) => {
                        format!(
                            "Server {}\nServer stdout:\n{}\nServer stderr:\n{}",
                            output.status,
                ...
                let log_infos = match self.log_file_contents() {
                    Ok(contents) => {
                        format!("Server logs:\n{contents}")
                    }
                    Err(err) => {
            ...
            "Server information no longer available (it has been reaped before)".to_string()
        }
    }
    ...
    pub fn stop_with_infos(&mut self) -> String {
